...
/*   Updated: 2020/08/06 18:10:43 by junhypar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf.h"

char	*x_sequens_4(char *num, t_calcul con)
{
	int		len;
	int		size;
	char	*out;
...
void	x_sequens_3(char *num, t_calcul con, char *out)
{
	int	len;

	len = ft_strlen(num);
	if (con.min == 1)
		x_sequens_5(len, out, num);
...
int		x_sequens_2(char *num_c, t_calcul con)
{
	char	*out;
	int		size;
	...
	if (con.dotafter != 0 && ft_strlen(num_c) <= con.dotafter)
		num_c = x_sequens_4(num_c, con);
	x_sequens_3(num_c, con, out);
	ft_putchar(out);
	size = ft_strlen(out);
	free(out);
	out = 0;
...
int		x_sequens_1(long long num_i, t_calcul con)
{
	char	*num_c;
	int		len;
	int		len2;
